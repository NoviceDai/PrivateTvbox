var rule = {
    ...
    二级: `js:
        ...
        let playFrom = [];
        let playUrls = [];
        appendGroup(playFrom, playUrls, '候选直链', directs);
        appendGroup(playFrom, playUrls, '外链解析', ninjas);
        appendGroup(playFrom, playUrls, '网页解析', parses);
        if (playFrom.length > 0) {
            appendGroup(playFrom, playUrls, '预览候选', previews);
        }
        VOD = {
            vod_name: title,
